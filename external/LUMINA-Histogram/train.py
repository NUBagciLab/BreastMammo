...
from dataset import BreastMammo_density, DenseMammo_density, LUMINA_Density, MultiImagesDataset
...
def load_data(args):
    ...
    train_data1 = BreastMammo_density(root = os.path.join(args.data_path, 'BreastMammo_PNG'))
    train_data2 = DenseMammo_density(root = os.path.join(args.data_path, 'DenseMammo_PNG'))
    train_data = {key: train_data1[key] + train_data2[key] for key in train_data1}
    
    # Dynamically loop through and append the synthetic data pools
    for alpha in [25, 50, 75, 100]:
        synth_iu = BreastMammo_density(
            root=os.path.join(args.data_path, f"BreastMammo_Histogram{alpha}")
        )
        synth_bmsp = DenseMammo_density(
            root=os.path.join(args.data_path, f"DenseMammo_Histogram{alpha}")
        )
        for key in train_data:
            train_data[key] += synth_iu[key] + synth_bmsp[key]
            
    test_data = LUMINA_Density(root = os.path.join(args.data_path, 'LUMINA_PNG'))
    train_ds = MultiImagesDataset(train_data, transform=transform_train)
    test_ds = MultiImagesDataset(test_data, transform=transform_test)
    print(f"There are {len(train_data['label'])} training and {len(test_data['label'])} testing samples.")
...
def test_fn(dataloader, model, loss_fn, device):
    ...
    with torch.no_grad(): 
        ...
        for X, y in progress_bar:
            ...
            X, y = [x.to(device) for x in X], y.to(device)
            pred = model(X)
            pred_all.extend(torch.softmax(pred, dim=1).cpu().numpy())
            result_all.extend(pred.argmax(1).cpu().numpy())
            loss = loss_fn(pred, y)
            total_loss += loss.item()
            correct = (pred.argmax(1) == y).type(torch.float).sum().item()
            total_correct += correct
    ...
    auc = roc_auc_score(y_all, pred_all, multi_class='ovr')
    f1 = f1_score(y_all, result_all, average='macro')
    return {'loss': total_loss/batch_count, 'acc': total_correct/sample_count, 'auc': auc, 'f1': f1}, {'true': y_all, 'pred': pred_all, 'result': result_all}
...
if __name__ == "__main__":
    ...
    best_ind = {'loss': 0, 'acc': 0, 'auc': 0, 'f1': 0}
    for epoch in range(args.epochs):
        ...
        for metric in ['loss', 'acc', 'auc', 'f1']:
            log['test_'+metric].append(epoch_log[metric])
        print(f"Epoch {epoch+1} train loss {log['train_loss'][-1]:.4f} acc {log['train_acc'][-1]:.4f}")
        print(f"test loss {log['test_loss'][-1]:.4f} acc {log['test_acc'][-1]:.4f} auc {log['test_auc'][-1]:.4f} f1 {log['test_f1'][-1]:.4f}")
        ...
        for metric, metric2 in (['acc', 'f1'], ['f1', 'acc'], ['auc', 'acc']): # save model when metric improves or both metric and metric2 are maximized. The second condition avoids one best again but another worse
