...
from dataset import BreastMammo, ImageDataset, get_fold
...
def load_data(args):
    ...
    data = BreastMammo(root = args.data_path)
    train_data, test_data = get_fold(data, fold = args.fold)
    train_ds = ImageDataset(train_data, transform=transform_train)
    test_ds = ImageDataset(test_data, transform=transform_test)
    print(f"There are {len(train_data['label'])} training and {len(test_data['label'])} testing images.")
