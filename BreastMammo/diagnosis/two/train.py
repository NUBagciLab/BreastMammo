...
from dataset import BreastMammo, MultiImagesDataset, get_fold
...
def load_data(args):
    ...
    data = BreastMammo(root = args.data_path, view = 2)
...
def train_fn(dataloader, model, loss_fn, optimizer, device):
    ...
    for batch, (X, y) in enumerate(progress_bar):
        X, y = [x.to(device) for x in X], y.to(device).unsqueeze(1)
        pred = model(X)
        loss = loss_fn(pred, y)
        loss.backward()
        optimizer.step()
